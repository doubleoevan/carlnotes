...
// skips are non-events. an all-skipped Sources topic still succeeds
test("toScanSummary treats skipped Sources as non-failures", () => {
	expect(toScanSummary([{ status: "skipped" }]).status).toBe("succeeded")
})

// only the Source that reports a fallbackMode is traced as degraded and the Scan still succeeds
