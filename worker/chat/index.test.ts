// chat briefing and history-compaction tests. they run with Langfuse keys unset,
...
import type { ChatContext } from "./retrieve"

// a context with one finding, one scan note, and no attachments, for cases to override
...
// a reader's own kept material reaches the model in its own labeled section
test("the reader's kept chat attachment context is interpolated", async () => {
...
// the written briefing includes every interpolated value and leaves no placeholder behind.
// the question itself is interpolated as a message, not template text, so it never appears here
test("the briefing interpolates the topic, findings, and notes", async () => {
...
// a non-owner's context includes no attachment material, so the briefing says there is none instead of omitting the section
...
test("the owner's attachment context is interpolated", async () => {
	const context = chatContext({ attachmentContext: "The reader's resume mentions Rust." })
	const { prompt } = await buildChatPrompt(context)
	expect(prompt).toContain("The reader's resume mentions Rust.")
})

// links are welcome from the material or a search, and a remembered source gets searched instead of being guessed
test("the briefing restricts links to URLs that the chat turn actually holds", async () => {
	const { prompt } = await buildChatPrompt(chatContext())
	expect(prompt).toContain("never one you remember")
	expect(prompt).toContain("run a quick search and link what it returns")
})

// the finding's url is included in the briefing, so a reply can link the finding it cites
test("the briefing includes each finding's url", async () => {
	const { prompt } = await buildChatPrompt(chatContext())
...
test("the briefing marks the retrieved material as data, not instructions", async () => {
	const { prompt } = await buildChatPrompt(chatContext())
...
// so the reader knows what came from the topic and what came from the model
test("the briefing invites general knowledge with the boundary marked", async () => {
	const { prompt } = await buildChatPrompt(chatContext())
	expect(prompt).toContain("General knowledge is fair game")
	expect(prompt).toContain("Mark the boundary")
})

// every chat turn can search the live web, so the briefing always names the tool
test("the briefing always includes the web access note", async () => {
	const { prompt } = await buildChatPrompt(chatContext())
	expect(prompt).toContain("searchWeb tool")
})

// the conversation is interpolated as messages, so the briefing points the model at it for references
test("the briefing tells the model the conversation follows", async () => {
	const { prompt } = await buildChatPrompt(chatContext())
...
test("attachments fold into the newest message as parts", () => {
	const messages = toModelMessages([{ question: "earlier", answer: "reply" }], "what is this?", [
		...
		{ kind: "image", name: "shot.png", data: "data:image/png;base64,AAA", keep: false },
	])
...
test("the briefing explains attachment notes so a real reading is never denied", async () => {
	const { prompt } = await buildChatPrompt(chatContext())
