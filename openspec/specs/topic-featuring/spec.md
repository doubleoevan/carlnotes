# topic-featuring Specification

## Purpose
TBD - created by archiving change admin-topic-feature-order. Update Purpose after archive.
## Requirements
...
A Topic that is not public SHALL NOT hold a feature order. A request to rank one SHALL be rejected, naming that rule, rather than silently ignored.
...
- **THEN** it is rejected, and no Topic's feature order changes
...
- **THEN** it is rejected as a conflict, and no Topic's feature order changes
