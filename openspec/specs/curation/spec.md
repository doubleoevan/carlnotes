...
### Requirement: Tiered LLM scoring produces Findings with why-summaries

Curation SHALL score each fetched survivor against the topic's effective context with a cheap-tier model routed through LiteLLM. A survivor whose first-pass score is at or above the promotion threshold SHALL be re-scored by a premium-tier model that also writes a one-line why-summary. Curation SHALL upsert one Finding per `(topic, resource)` carrying the `signal_score`, the `why_summary`, and the `scan_id`. Only curation writes Findings; adapters never do.
...
- **THEN** a Finding is written for `(topic, resource)` with the premium `signal_score`, a non-empty `why_summary`, and the current `scan_id`
...
- **THEN** it is not re-scored by the premium tier and consumes no premium-tier spend
...
- **THEN** `kept_count` equals the number of Findings written, `filtered_count` equals the number of Resources dropped by dedupe or the embed-filter, and `ai_summary` is set
