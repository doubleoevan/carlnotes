...
### Requirement: Bluesky ingester emits the articles an account links to
...
- **THEN** `blueskyIngester` reads that account's posts from the public appview without credentials and emits one Resource per linked article, each carrying the link card's title and description
...
- **THEN** the emitted Resource's `kind` is `watch`, the same kind the video's own Source would emit, so the two dedupe

#### Scenario: The sharing post's likes stand as engagement

- **WHEN** `blueskyIngester` emits a Resource for a linked article
...
- **THEN** both emit `watch`, so the two dedupe to a single Resource
