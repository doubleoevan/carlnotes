...
One additive enum value: `shared/enums.ts` gains `bluesky`, then `bun run db:generate` emits `ALTER TYPE "public"."source_kind" ADD VALUE 'bluesky'` and `bun run db:migrate` applies it — no backfill, no table change, every existing Source row untouched. Deploy is the migration plus the app code; no credential is required for the ingester to work, so there is no ordering constraint. Rollback is reverting the code; the enum value stays behind unused. Verification gate: `bunx biome check . && bunx tsc -b && bun test`, all offline.
