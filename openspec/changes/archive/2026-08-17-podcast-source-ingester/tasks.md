...
- [x] 3.1 Teach `parseFeed` in `worker/ingest/feed.ts` the `podcast:transcript` custom field, picking the plain-text or WebVTT entry when several are listed and the first otherwise, and set `transcriptUrl` on the found Resource
...
- [x] 8.1 Archive `youtube-caption-transcripts` before this change: it RENAMEs the curation fetch requirement, and this change's curation delta modifies the requirement under its new name with the identical merged text
