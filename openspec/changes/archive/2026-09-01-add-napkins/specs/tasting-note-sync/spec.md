...
Note note content syncs collaboratively over the app's existing SSE machinery — Yjs snapshots and updates over plain HTTP GET/POST plus the chat-style SSE stream — so concurrent editors converge with no websockets, no Hocuspocus, and no new infrastructure.
