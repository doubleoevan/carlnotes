...
- **Code:** new `worker/curation.ts` (+ `.test.ts`) and `worker/firecrawl.ts`; edits to `worker/scan.ts` (run curation before closing the Scan), `worker/llm.ts` (`embedModel`/`scoreModel`), the four adapters (native snippet), and `db/schema.ts`.
