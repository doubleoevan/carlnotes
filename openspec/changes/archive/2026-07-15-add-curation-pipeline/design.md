...
3. **Seams.** Extend `worker/llm.ts` with `embedModel()` and `scoreModel()` (same fail-fast env guard). Add `worker/scrape.ts` (`fetchContent(url)` via raw `fetch`, timeout-bounded, returns content or throws).
