...
Widening the gate alone would make the second problem worse. `generateAttachmentContext` returns a
...
- Resolve the content type at the trust boundary instead of trusting `file.type`. A browser reports
  ...
  the gate then rejects. Fall back to the filename extension, the way `toAttachment` already does
  with `TEXT_FILE_SUFFIXES`, and settle on one canonical type that storage, the extractor, and
  `toStoredFileHeaders` all agree on.
- Give table files deterministic table text instead of a summary: a computed header line
...
- **BREAKING for prompt shape:** label each attachment's context in `buildTopicScanContext` with a
  line naming its file, and require every generative consumer of the merge to interpolate it as
  untrusted, where the prompt writer's per-call nonce delimiter holds the whole merge as data. The
  writer strips any delimiter tag a value includes, so the fence lives there and nowhere else.
- Decide what an unscreened table file does. `screenText` fails open by design, and 64k
  ...
  the prompt. Table text has no such step.
...
- Settle the chat picker. `CHAT_FILE_PICKER_ACCEPT` derives from `FILE_PICKER_ACCEPT`, so widening
  the topic picker makes both chat composers offer a file `toAttachment` then rejects with a toast.
...
- `table-attachments`: what a table file is, how its table text is computed and limited,
...
- `injection-defense`: attachment contexts are nonce-fenced where they merge, and an unscreened
  table file is held to a stated rule.

## Impact

- `worker/attach.ts`: the type gate, `extractText`, and the `buildTopicScanContext` merge.
- `worker/workflows/process-attachment-activities.ts`: the table branch, the truncation marker,
...
- Scan cost. `buildTopicScanContext` merges every ready attachment's context into every scan, so a
  table text is a recurring per-scan token cost where a summary was a one-time ~200 characters.
