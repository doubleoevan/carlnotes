...
A Source still pending when the bound passes SHALL be left for a later Scan, so a page that never loads delays the first Scan by the bound rather than by the screening workflow's full ceiling.
...
A Source row is written before its screening workflow starts, so a failure in that gap can leave a Source pending with nothing behind it. Such a Source SHALL be left pending and picked up again by the existing scheduled sweep, rather than by a second reclaim path of its own. Because the workflow is keyed on the Source, a restart for a Source whose screen is genuinely running SHALL be refused rather than duplicating the work.
...
- **THEN** the start is refused and the running screen is left alone
...
- **THEN** the failure is reported, the Source stays pending and therefore stays inert, and the next sweep starts it
