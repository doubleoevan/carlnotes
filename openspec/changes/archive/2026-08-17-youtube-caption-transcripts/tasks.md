...
- [x] 8.6 Run `bun run smoke:review` against a Topic with a video Source and read the fetch stage's output, confirming a video's Finding cites what the video says and `stage_costs.fetch` did not grow for it. Needs Doppler and a seeded Topic, so it is left for a real environment.
