...
The invites table SHALL have exactly one target — a Topic or a Team — with the token lifecycle (limits, expiry, revocation, rejections) unchanged from the invite-links change. Creating a team invite SHALL require membership. Accepting a leader's invitation SHALL write a membership row and the muted Subscriptions. Accepting a member's invitation SHALL write a not-yet-active membership row instead — a join request a leader activates like any other, with the muted Subscriptions written at activation — so members invite whoever and a leader still approves who gets in. Either way the accepter is sent to the team page. Removing a member SHALL revoke access on the next request and leave their authored room messages in place, still attributed.
...
- **WHEN** someone accepts an invitation a non-leader member created
