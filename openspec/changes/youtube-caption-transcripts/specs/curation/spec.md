...
- FROM: `### Requirement: Survivors are fetched via Firecrawl with a snippet fallback`
- TO: `### Requirement: Survivors are fetched by the path their kind selects, with a snippet fallback`
...
#### Scenario: A non-304 conditional response falls through to a fresh fetch

- **WHEN** the conditional GET returns any status other than `304`
- **THEN** curation abandons the probe result and performs the normal fetch for that Resource's path
...
#### Scenario: Stale content with no stored validators skips straight to a fresh fetch
