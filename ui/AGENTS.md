...
- `components/table/` — the page tables; `lib/` — `utils.ts`, small pure client-side helpers, and
  `visitAnalytics.ts`, which starts the browser's analytics and reports a page view per route change.
...
- `stores/` — app state one module owns and any component may read, named `<thing>Store.ts`. Each keeps
  its value in module scope and takes its listener set from `storeListeners.ts`, the one place the
  `useSyncExternalStore` contract lives, so two components read one live value with no provider
  wrapping them. Reach for a store only when state outlives a page or crosses the tree; `useState` and
  `providers/` cover everything else.
