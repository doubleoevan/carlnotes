...
export function TopicFeedSort() {
	// the sort lives in the topic feed context
	const { sort, setSort, hasTopicFeed } = useTopicFeed()
	const [isOpen, setIsOpen] = useState(false)
	// a page with no findings hides the control
	if (!hasTopicFeed) {
		return null
	}
	return (
		<Popover open={isOpen} onOpenChange={setIsOpen}>
			...
			<PopoverContent align="end" sideOffset={13} className="w-40" bodyClassName="p-1">
				...
				{TOPIC_FINDING_SORTS.map((sortOption) => {
					...
					return (
						<button
							...
							className={cn(MENU_OPTION_CLASS, sortOption === sort && MENU_OPTION_SELECTED_CLASS)}
						>
