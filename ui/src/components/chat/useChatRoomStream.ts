...
import type { ChatRoomMessage, ChatRoomMessagePage, RoomTopicToolCalls } from "@shared/contracts"
...
export function useChatRoomStream(
	...
	chatHandlers: {
		...
		// the toast lines and the proposal carl's tools left in his chat turn, sent after his chat message
		onTopicToolCalls: (roomToolCalls: RoomTopicToolCalls) => void
	},
): void {
	...
	useEffect(() => {
		...
		const connect = (): void => {
			...
			// the toast lines and the proposal carl's tools left follow his chat message
			eventSource.addEventListener("topicToolCalls", (event) => {
				lastResponseTime = Date.now()
				chatHandlersRef.current.onTopicToolCalls(JSON.parse(event.data) as RoomTopicToolCalls)
			})
