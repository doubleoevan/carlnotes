...
export function toChromeIntentUrl(pageUrl: string): string {
	// the scheme is carried by the intent's own end, so the url it wraps goes in without one.
	// it is read off the page instead of assumed, so the intent opens the address the visitor is actually on
