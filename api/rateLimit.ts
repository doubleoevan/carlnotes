...
// what identifies a tool caller: the user a session or an accepted bearer token names, else the forwarded chain
export type RateLimitToolCaller = { userId: string | null; forwardedFor: string | null }
 ...
 */
export function toRateLimitKey(toolCaller: RateLimitToolCaller, trustedProxyCount: number): string {
	if (toolCaller.userId) {
		return `user:${toolCaller.userId}`
	}
	const clientAddress = toClientAddress(toolCaller.forwardedFor, trustedProxyCount)
...
// the tool caller a request identifies: the session user, else the user an mcp bearer token resolved to, else the chain
...
export const toolCallerRateLimiter = rateLimiter<AppEnv>({
	...
	keyGenerator: (context) => toRateLimitKey(toRateLimitToolCaller(context), trustedProxies.length),
