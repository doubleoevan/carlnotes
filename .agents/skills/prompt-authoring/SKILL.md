...
- **Reader-facing model text renders through a sanitized markdown subset with allowlisted links.** Formatting (bold, lists, headings) is fine to ask for. A link only works when its destination is a kept Finding's own url — never ask for a link to anything else, since it would render as inert text anyway.
