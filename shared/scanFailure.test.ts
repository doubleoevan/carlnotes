// scan failure label tests: budget-wall detection and the user-facing failure line
import { expect, test } from "bun:test"
import { isBudgetError, toScanFailureLabel } from "./scanFailure"

// the litellm spend-limit error counts as a budget failure. anything else does not
test("isBudgetError matches the litellm spend-limit error", () => {
...
test("toScanFailureLabel humanizes known failures and strips wrapper noise", () => {
	...
	expect(toScanFailureLabel("Failed after 3 attempts. Last error: AI_APICallError: connection rejected")).toBe(
		"connection rejected",
	)
